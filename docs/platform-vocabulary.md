...
Emits x86-64 assembly for `main` programs with integer/boolean return,
arithmetic (`+`, `-`, `*`, `/`, `%`), all comparisons, logical operators
(`and`, `or`, `not`), `let` locals, assignment, `if`/`else`, and `while`
loops. No function parameters, `elif`, extern calls, structs, or strings yet.

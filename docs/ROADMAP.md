...
| 4a    | done     | d8f76aa        |
| 4b    | done     | 625c740        |
| 5     | in progress (extern calls + 18-case backend subset + narrow i32 array indexing smoke + narrow i32 variable-index array smoke + narrow local integer struct field smoke + narrow local byte string array smoke + narrow local struct array field smoke + narrow byte array indexed assign/read smoke) | -        |
| 6     | not started | -            |
| 7     | not started | -            |
| 8     | not started | -            |
| 9     | not started | -            |
...
- 5e: C emitter in ETL (smoke DONE; void + return-valued extern calls DONE; narrow i32 array indexing DONE at fa722e8; narrow i32 variable-index array smoke DONE at 6df84e6; narrow local integer struct field smoke DONE at 902b736; narrow local byte string array smoke DONE at ed3d8de; narrow local struct array field smoke DONE at 6c54423; narrow byte array indexed assign/read smoke DONE at e45b1e8)
- 5f: c0→c1 builds c1; c1 compiles fixture corpus; behavior-equivalent diff (next; struct array field and byte array assign now proven; struct params/returns, nested structs, non-integer fields, i8 arrays for indexing, dynamic arrays, bounds checks, ASM/WAT parity, and broad self-compilation struct support remain)

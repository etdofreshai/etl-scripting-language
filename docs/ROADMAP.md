...
> C/WAT/ASM, a
> 22-fixture c1 equiv corpus including local fixed i32 array sum and loop
> fixtures, and an 18-case shared C/ASM/WAT backend subset.
...
- 5f: c0→c1 builds c1; c1 compiles fixture corpus; behavior-equivalent diff (in progress; 22 c1 corpus fixtures now pass including multi-function, `i32` parameter, recursive calls, local fixed i32 array summation returning 100, and local fixed i32 array loop indexing returning 49; variable-index i32 arrays, byte array indexed assignment, user-defined byte-array params, scalar bool/i8/byte params (C and WAT), struct array field read/write, by-value struct params, byte string variable-index reads, byte string extern C pointer params, multi-buffer byte strings, and C/ASM extern scalar bool/i8/byte parameter emission now proven; typed locals beyond `i32`, extern typed params beyond scalar bool/i8/byte, and struct returns remain)

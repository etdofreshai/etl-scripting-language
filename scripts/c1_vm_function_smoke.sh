...
cc -std=c11 -Wall -Wextra -Werror "$td/run_vm.c" runtime/etl_vm.c runtime/etl_string.c -I runtime -o "$td/run_vm"
...
run_case() {
  ...
  "$c1_harness_exe"
  if [ ! -s "$c1_c" ]; then
    echo "c1_vm_function_smoke: FAIL $name compiler-1 emitted no C" >&2
    exit 1
  fi
  ...
  if [ ! -s "$c1_bc_path" ]; then
    echo "c1_vm_function_smoke: FAIL $name compiler-1 emitted no bytecode" >&2
    exit 1
  fi
  local vm_status
  ...
  if [ "$c0_status" != "$expected" ] || [ "$c1_status" != "$expected" ] || [ "$vm_status" != "$expected" ]; then
    echo "c1_vm_function_smoke: FAIL $name expected=$expected c0=$c0_status c1=$c1_status vm=$vm_status" >&2
    exit 1
  fi
  echo "c1_vm_function_smoke: PASS $name (exit $expected on c0, c1/C, c1/VM)"
}

run_case "two_fn_add" "fn add(a i32, b i32) i32 ret a + b end fn main() i32 ret add(20, 22) end" "42"
run_case "recursive_fact" "fn fact(n i32) i32 if n <= 1 ret 1 end ret n * fact(n - 1) end fn main() i32 ret fact(5) end" "120"
run_case "helper_of_helper" "fn twice(x i32) i32 ret x * 2 end fn quad(x i32) i32 ret twice(twice(x)) end fn main() i32 ret quad(7) end" "28"

echo "c1_vm_function_smoke: ok (c0/c1-C/c1-VM equivalence over direct calls and recursion)"

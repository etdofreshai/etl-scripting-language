...
if [ "$status" -ne 134 ]; then
  echo "c1_emit_c_smoke: FAIL - expected emitted C length exit 134, got $status" >&2
  exit 1
fi

echo "c1_emit_c_smoke: ok (fn main() i32 ret 1 + 2 * (9 - 4) end -> stdbool+etl_string+etl_dynarr includes, prototype plus arithmetic return C)"

...
source_text='type Item structure left integer right integer end fn main() i32 let items Item[2] items[0].left = 19 let i i32 = 1 items[i].right = 23 ret items[0].left + items[i].right end'
source_len="$(printf "%s" "$source_text" | wc -c)"
asm_out="$td/struct_array.s"
obj_out="$td/struct_array.o"
native="$td/struct_array_native"
...
if [ ! -s "$asm_out" ]; then
  echo "c1_asm_struct_array_smoke: FAIL - compiler-1 harness did not write ASM" >&2
  exit 1
fi

for fragment in 'mov $19, %rax' 'mov %rax, -16(%rbp)' 'mov $23, %rax' 'imul $8, %rax' 'mov %rax, -12(%rbp,%rcx,1)' 'mov -16(%rbp), %rax' 'mov -12(%rbp,%rax,1), %rax' 'add %rcx, %rax'; do
  if ! grep -q "$fragment" "$asm_out"; then
    echo "c1_asm_struct_array_smoke: FAIL - ASM missing expected fragment: $fragment" >&2
    cat "$asm_out" >&2
    exit 1
  fi
done
...
if ! as --64 -o "$obj_out" "$asm_out"; then
  echo "c1_asm_struct_array_smoke: FAIL - assembler failed" >&2
  cat "$asm_out" >&2
  exit 1
fi
...
if [ "$status" -ne 42 ]; then
  echo "c1_asm_struct_array_smoke: FAIL - expected native exit 42, got $status" >&2
  cat "$asm_out" >&2
  exit 1
fi

echo "c1_asm_struct_array_smoke: ok (native exit 42)"
